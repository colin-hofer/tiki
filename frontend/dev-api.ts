  ...
  return {
    ...
    async configureServer(vite) {
      ...
      if (!Number.isInteger(port) || port < 1 || port > 65535)
        throw new Error('TIKI_DEV_API_PORT must be between 1 and 65535');
      ...
      async function restart(initialize = false) {
        ...
        if (initialize) {
          server.config.logger.info(
            '[tiki] First run: choose a password for dev@tiki.local. Existing accounts are preserved.',
          );
          await run(
            binary,
            [
        ...
        const child = (api = spawn(
          binary,
          ['serve', '--db', database, '--listen', `127.0.0.1:${port}`],
            ...
            .catch((err) => {
              if (!closing)
                server.config.logger.error(
                  `[tiki] ${err.message}. Fix the error and save to retry.`,
                );
            });
        }, 150);
      });
