...
async function mock(context: BrowserContext) {
  const state = {
    ...
    failProfile: false, expired: false, logout: 0,
    profiles: [] as { name: string }[], passwords: [] as { current_password: string; new_password: string }[],
  ...
  await context.route('**/api/v1/**', async route => {
    ...
    if (path === '/auth/login') { state.expired = false; return reply({ user: state.user, session_token: 'new-test-session', expires_at: 9999999999 }); }
    if (state.expired) return reply({ error: { code: 'unauthorized', message: 'Session expired' } }, 401);
    if (path === '/auth/me' && request.method() === 'PATCH') {
      state.profiles.push(request.postDataJSON());
      if (state.failProfile) return reply({ error: { code: 'internal', message: 'Could not save profile' } }, 500);
      state.user.name = request.postDataJSON().name; return reply(state.user);
    }
    if (path === '/auth/me') return reply(state.user);
    if (path === '/auth/password') {
      const body = request.postDataJSON(); state.passwords.push(body);
      if (body.current_password !== 'correct-password') return reply({ error: { code: 'validation', message: 'current password is incorrect' } }, 400);
      await state.beforePasswordReply?.(); state.expired = true; return reply({ password_changed: true });
    }
    if (path === '/auth/logout') { state.logout++; state.expired = true; return reply({ logged_out: true }); }
    if (path === '/board') return reply({ columns: Object.fromEntries(statuses.map(status => [status, { items: [] }])), users: { users: [state.user] }, tags: { tags: [] } });
...
test('account menu supports arrows and vim keys, and profile saves persist for viewers', async ({ page, context }) => {
  const state = await mock(context); await page.goto('/');
...
test('profile errors preserve the name draft and permit retry', async ({ page, context }) => {
  const state = await mock(context); state.failProfile = true; await page.goto('/');
  ...
  await page.getByRole('button', { name: 'Save name' }).click();
  await expect(page.getByRole('dialog', { name: 'Edit profile' }).getByRole('status')).toHaveText('Name updated.');
});

test('password form validates confirmation, keeps incorrect-password errors local, then signs out', async ({ page, context }) => {
  const state = await mock(context); await page.goto('/');
  await page.getByRole('button', { name: 'Account menu' }).click();
  ...
  state.beforePasswordReply = () => page.evaluate(() => window.dispatchEvent(new Event('tiki:expired')));
  await dialog.getByRole('button', { name: 'Change password', exact: true }).click();
  await expect(page.getByRole('button', { name: 'Sign in', exact: true })).toBeVisible();
  await expect(page.getByText('Password changed. Sign in again with your new password.')).toBeVisible();
  ...
  expect(state.passwords.at(-1)).toEqual({ current_password: 'correct-password', new_password: 'replacement-password' });
});

test('account actions fit mobile and sign out clears the session', async ({ page, context }) => {
  const state = await mock(context);
  await page.setViewportSize({ width: 390, height: 740 }); await page.goto('/');
  const trigger = page.getByRole('button', { name: 'Account menu' });
  await expect(trigger).toBeInViewport(); await trigger.click();
