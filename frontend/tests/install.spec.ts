...
async function mock(context: BrowserContext, platforms: string[]) {
  ...
  await context.route('**/api/v1/**', async route => {
    ...
    if (path === '/board') return reply({ columns: Object.fromEntries(['backlog', 'todo', 'in_progress', 'code_review', 'blocked', 'complete', 'void'].map(status => [status, { items: [] }])), users: { users: [user] }, tags: { tags: [] } });
...
test('CLI and skill setup copy workspace commands without exposing a session', async ({ page, context }, testInfo) => {
  ...
  await expect(dialog.getByRole('link', { name: 'View install script' })).toHaveAttribute('href', '/api/v1/cli/install.sh');
  await expect(dialog).toContainText('macOS Apple Silicon');
  await dialog.getByRole('button', { name: 'Copy sign-in command' }).click();
  expect(await page.evaluate(() => navigator.clipboard.readText())).toBe(`tiki auth login --server '${origin}' --email 'o'\\''hara@example.test'`);
  await page.screenshot({ path: testInfo.outputPath('setup-cli-desktop.png'), animations: 'disabled' });
  ...
  await expect(dialog.getByRole('tab', { name: 'Agent skill', exact: true })).toHaveAttribute('aria-selected', 'true');
  ...
  await expect(dialog.getByRole('link', { name: 'Read SKILL.md' })).toHaveAttribute('href', '/api/v1/skills/tiki/SKILL.md');
  await page.screenshot({ path: testInfo.outputPath('setup-skill-desktop.png'), animations: 'disabled' });
  await page.keyboard.press('Escape');
  await expect(dialog).toHaveCount(0);
  await expect(trigger).toBeFocused();
});

test('mobile setup keeps the skill available without CLI binaries', async ({ page, context }, testInfo) => {
  await mock(context, []);
  ...
  await expect(dialog.getByRole('textbox', { name: 'Skill install command', exact: true })).toHaveValue(/ --skill$/);
  expect(await dialog.evaluate(element => element.scrollWidth <= element.clientWidth)).toBe(true);
  await page.screenshot({ path: testInfo.outputPath('setup-skill-mobile.png'), animations: 'disabled' });
...
test('clipboard failure leaves commands available for manual copying', async ({ page, context }) => {
  await mock(context, ['linux-amd64']);
  await context.addInitScript(() => Object.defineProperty(navigator, 'clipboard', { value: { writeText: async () => { throw new Error('denied'); } } }));
  await page.goto('/');
  ...
  await expect(dialog.getByRole('alert')).toHaveText('Clipboard unavailable. Select the command and copy it manually.');
  const command = dialog.getByRole('textbox', { name: 'CLI install command', exact: true });
  await expect(command).toBeVisible();
  await command.click();
  expect(await command.evaluate((element: HTMLTextAreaElement) => element.selectionEnd - element.selectionStart)).toBeGreaterThan(20);
  await dialog.getByRole('tab', { name: 'Agent skill' }).click();
