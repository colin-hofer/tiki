...
async function mock(context: BrowserContext) {
  const state = { valid: true, claims: 0, me: 0, role: '', user: admin, users: [{ ...admin }, { ...member }] as User[], failChanges: false };
  await context.route('**/api/v1/**', async route => {
    ...
    if (path.startsWith('/users/')) {
      if (state.failChanges) return reply({ error: { code: 'conflict', message: 'Could not save role' } }, 409);
      const user = state.users.find(u => u.id === path.split('/')[2])!;
      if (route.request().method() === 'DELETE') user.removed_at = 1;
      else user.role = body.role;
      return reply(user);
    }
    if (path === '/board') return reply({ columns: {}, users: { users: state.users }, tags: { tags: [] } });
...
test('people management changes roles, handles failed saves, and removes access', async ({ page, context }) => {
  const state = await mock(context);
  await context.addInitScript(() => sessionStorage.setItem('tiki.session', 'admin-session'));
  await page.goto('/');
  ...
  await expect(dialog.getByRole('combobox', { name: 'Role for Admin', exact: true })).toBeDisabled();
  ...
  await expect(dialog.getByText('Removed · Invite again to restore', { exact: true })).toBeVisible();
