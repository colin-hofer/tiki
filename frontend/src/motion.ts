...
export function pin(id: string, rect: DOMRect, used: () => void) {
  pinned.set(id, { rect, used });
}
...
export function arrive(node: HTMLElement): TransitionConfig {
  ...
  if (drop) {
    pinned.delete(node.dataset.card || '');
  ...
  ghost.firstElementChild?.animate([{ boxShadow: lift }, { boxShadow: lift, offset: 0.7 }, {}], {
    duration: ms,
    easing: 'ease-out',
  });
  ghost
    .animate(
      ...
      { duration: ms, easing: 'cubic-bezier(.33, 1, .68, 1)' },
    )
    .finished.then(
      () => ghost.remove(),
      () => ghost.remove(),
...
export function panel(
  ...
  return {
    duration: duration(full ? 260 : direction === 'out' ? 160 : 220),
    easing: cubicOut,
    css: (t, u) =>
      full
        ? `transform: translateX(${u * 100}%)`
        : `transform: translateX(${u * 24}px); opacity: ${t}`,
  };
}
