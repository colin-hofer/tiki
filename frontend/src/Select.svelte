...
<script lang="ts">
  ...
  function keydown(event: KeyboardEvent) {
    ...
    if (!open) {
      if (['ArrowDown', 'ArrowUp', 'Enter', ' '].includes(key)) { consume(); void show(); }
      else if (key.length === 1 && !event.altKey && /\S/.test(key) && options.length) {
        consume(); void show().then(() => { const index = typeahead(key); if (index >= 0) scrollTo(index); });
      }
      return;
    }
    ...
    else if (key.length === 1) { const index = typeahead(key); if (index >= 0) scrollTo(index); }
  }

  $effect(() => {
    if (!open) return;
    const outside = (event: PointerEvent) => { if (!popup.contains(event.target as Node) && !trigger.contains(event.target as Node)) hide(false); };
    const scroll = (event: Event) => { if (!popup.contains(event.target as Node)) hide(false); };
    document.addEventListener('pointerdown', outside, true);
    window.addEventListener('scroll', scroll, true);
    window.addEventListener('resize', place);
    return () => { document.removeEventListener('pointerdown', outside, true); window.removeEventListener('scroll', scroll, true); window.removeEventListener('resize', place); };
  });
  $effect(() => { if (disabled) hide(false); });
</script>

<button bind:this={trigger} type="button" class={`select-trigger ${variant}`} class:open class:has-value={!showPlaceholder}
  role="combobox" aria-label={label} aria-haspopup="listbox" aria-expanded={open} aria-controls={`${id}-list`}
  aria-activedescendant={open ? `${id}-${active}` : undefined} title={title || undefined} {disabled}
  onclick={() => open ? hide() : void show()} onkeydown={keydown}>
  {#if showPlaceholder}
