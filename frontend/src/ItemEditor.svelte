<script lang="ts">
  ...
  $effect(() => {
    ...
    const changes = patch();
    if (!changed || !canWrite || suspended || saving || composing || conflict || error || !title.trim()) return;
  ...
  function save(): Promise<void> {
    ...
    if (destroyed || suspended || !canWrite || !base || !changed || !title.trim() || composing || conflict || error) return Promise.resolve();
  ...
  async function persist() {
    ...
    try {
      ...
      if (destroyed || deleted) return;
      if (e instanceof APIError && e.status === 404) { onmissing(); return; }
  ...
  export async function settle() { if (inFlight) await inFlight; }
  ...
  async function loadActivity(more = false) {
    if (!item || deleted || activityBusy) return;
...
<aside class="detail" tabindex="-1" bind:this={panel} aria-label={`Item ${item.id}`}>
  ...
  <form class="editor-form" onsubmit={event => { event.preventDefault(); void flush(true); }} oncompositionstart={() => composing = true} oncompositionend={() => composing = false}>
    <div class="detail-content">
      ...
      {/if}
      {#if deleted}<div class="conflict" role="status"><strong>This ticket was deleted.</strong><p>{dirty ? "Your unsaved text is kept here. Copy anything you need before discarding the draft." : "You can close this panel."}</p>{#if dirty}<button type="button" class="small-button" onclick={() => { fill(item); onclose(); }}>Discard draft and close</button>{/if}</div>{/if}
      ...
      <textarea id="description" class="description-editor" placeholder="Description (Markdown)" bind:value={description} onblur={() => void save()} readonly={deleted} disabled={readonly && !deleted} spellcheck="false"></textarea>
      {#if base}
